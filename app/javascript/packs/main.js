...
$(document).ready(function () {
  ...
  $("#order-products").change(function(){
    var selectedOrder = $(this).children("option:selected").val();
    let url = window.location.href;
    var mapObj = {
      'order=asc': 'order=' + selectedOrder,
      'order=desc': 'order=' + selectedOrder,
    };

    var field = 'order';
    if(url.indexOf('order=') != -1){
      url = url.replace(/order=asc|order=desc/gi, function(matched){
        return mapObj[matched];
      });
    }
    else {
      if(url.indexOf('?') != -1){
        url = url + '&order=' + selectedOrder;
      }
      else {
        url = url + '?order=' + selectedOrder;
      }
